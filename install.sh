...
install_packages() {
  case "$(uname -s)" in
    Darwin)
      ...
      brew install git zsh starship fzf zoxide eza bat ripgrep
      brew install --cask font-jetbrains-mono-nerd-font
      ;;
    Linux)
      if command -v apt-get >/dev/null 2>&1; then
        sudo apt-get update
        sudo apt-get install -y git zsh curl fzf zoxide eza bat ripgrep
      else
...
# Claude Code Stop hook + chonk-collab CLI (see CLAUDE.md).
link_file "$DOTFILES_DIR/claude/hooks/codex-review.py" "$HOME/.claude/hooks/codex-review.py"
link_file "$DOTFILES_DIR/claude/hooks/codex-review.README.md" "$HOME/.claude/hooks/codex-review.README.md"
link_file "$DOTFILES_DIR/bin/chonk-collab" "$HOME/.local/bin/chonk-collab"
